...
class CollectionRequestViewSet(viewsets.ModelViewSet):
    ...
    # get requests as a user
    def get_queryset(self):
        user = self.request.user
        if user.is_superuser:
            return CollectionRequest.objects.all()

        return CollectionRequest.objects.filter(models.Q(to_user=user) | models.Q(from_user=user) )
 ...
 # ✅ ADDED: Approve collection request and create transaction
